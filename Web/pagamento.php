...
<?php echo $_SESSION['nome'] . " " . $_SESSION['cognome'];?>
  ...
  <ul class="navbar-nav ">
    <div class="row">
      ...
      <div class="col-xs-6">
        <li class="nav-item">
          <a class="nav-link" href="carrello.html">Cashout ></a>
        </li>
      </div>
    </div>
  </ul>
...
<div id="pagamento" class="row ">
  <div class="col-sm-12">
    <h2 class="text-center">Modalità di pagamento:</h2>
  </div>
    <div class="container ">
      <?php
      if(isset($_SESSION['indirizzo']) && $_SESSION['indirizzo']==="E cantoun dla pida: via del Maniero, 17"){ ?>
      <div class="col-sm-12">
        <div class="radio">
          <input type="radio" name="pagamento" value="sede" id="sede"> <label for="sede">E cantoun dla pida, via del Maniero, 17</label>
          <?php showCarta(); ?>
        </div>
      </div>
      <?php
    }else if(isset($_SESSION['indirizzo']) && $_SESSION['indirizzo']!=="E cantoun dla pida: via del Maniero, 17") {
        ?>
        <div class="col-sm-12">
          <div class="radio">
            <input type="radio" name="pagamento" value="fattorino" id="fattorino"> <label for="fattorino">Fattorino</label>
            <?php showCarta(); ?>
          </div>
        </div>
        <?php
      } else{
        header("location: consegna.php?error=");
      } ?>
      <?php
      if(isset($_GET['add'])){
        ?>
        <form action="php/addCard.php" method="post">
          <label>NumeroCarta : <input type="text" name="carta" value=""></label>
          <input type="submit" class="btn btn-outline-dark" name="submit" value="Aggiungi Carta">
        </form>
        <?php
      }else{
        ?><button type="button" class="btn btn-outline-dark" onclick="window.location='pagamento.php?add'">Aggiungi modalità di pagamento</button><?php
      }
       ?>
       <?php if(isset($_GET['error'])){
         ?><p style="color:red">Selezionare un metodo di pagamento.</p><?php
       } ?>
    </div>
  </div>
      ...
      <button id="left" type="button" class="btn btn-outline-dark" onclick="window.location='consegna.php'"><< Indirizzo e ora di consegna</button>
      </div>
      </div>
      <div class="row">
        <div class="col-sm-12 text-right">
      <input id="right" type="submit" class="btn btn-outline-dark" value="Riepilogo e conferma >>">
      </div>
      </div>
</form>
...
function showCarta(){
  ...
  $email= $_SESSION['email'];
  $sql = "SELECT carta FROM pagamento WHERE email= '$email'";
  $result = $conn->query($sql);
  $i=1;
  if ($result->num_rows > 0) {
      while($row = $result->fetch_assoc()) {
        ?><br><input type="radio" name="pagamento" value="<?php echo $row['carta'];?>" id="carta <?php echo $i; ?>">
        <label for="carta <?php echo $i; ?>"> <?php echo "Carta ". "$i" . ": " . $row['carta'];?> </label><?php
        $i++;
      }
  }
}
