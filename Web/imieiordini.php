...
<body class="bg-light">
  ...
  <div class="jumbotron jumbotron-fluid">
    <div class="container">
      ...
      <h2 class="text-center">I mie ordini</h2>
    </div>
  </div>
    ...
    <div class="col-sm-12">
      <div class="container">
        <div class="table-responsive">
            ...
            $sql = "SELECT carrello, orario, prezzo, evaso FROM ordine WHERE email = '$email' ORDER BY orario DESC";
            ...
            if ($result->num_rows > 0) {
              ?><table class="table table-dark  text-center">
                ...
                while($row = $result->fetch_assoc()) {
                    $id = $row['carrello'];
                    $sql = "SELECT id, prodotto, categoria, quantita FROM carrello WHERE id = '$id'";
                    $res = $conn->query($sql);
                    if ($res->num_rows > 0) {
                      ?><tr><td><?php echo substr($row['orario'],8,2) . substr($row['orario'],4,4) . substr($row['orario'],0,4) . " alle " .
                       substr($row['orario'],11,5); ?> </td><td><table class="table bg-dark"><?php
                      while($r = $res->fetch_assoc()) {
                        $categoria = $r['categoria'];
                        $prodotto = $r['prodotto'];
                        if ($categoria==1){
                            $sql = "SELECT nome FROM piada WHERE id = '$prodotto'";
                            $risultato = $conn->query($sql);
                            if ($risultato->num_rows > 0) {
                              while($riga = $risultato->fetch_assoc()) {
                                ?><tr><td><?php echo $riga['nome'] . " x" . $r['quantita']; ?></td></tr><?php
                              }
                            }
                          } else if($categoria==2){
                            $sql = "SELECT nome FROM rotolo WHERE id = '$prodotto'";
                            $risultato = $conn->query($sql);
                            if ($risultato->num_rows > 0) {
                              while($riga = $risultato->fetch_assoc()) {
                                ?><tr><td><?php echo $riga['nome'] . " x" . $r['quantita']; ?></td></tr><?php
                            ...
                            $sql = "SELECT nome FROM cassone WHERE id = '$prodotto'";
                            $risultato = $conn->query($sql);
                            if ($risultato->num_rows > 0) {
                              while($riga = $risultato->fetch_assoc()) {
                                ?><tr><td><?php echo $riga['nome'] . " x" . $r['quantita']; ?></td></tr><?php
                            ...
                            $sql = "SELECT nome FROM bibita WHERE id = '$prodotto'";
                            $risultato = $conn->query($sql);
                            if ($risultato->num_rows > 0) {
                              while($riga = $risultato->fetch_assoc()) {
                                ?><tr><td><?php echo $riga['nome'] . " x" . $r['quantita']; ?></td></tr><?php
                      ...
                      ?></table></td><td><?php echo $row['prezzo']; ?></td><?php
                      ?><td><?php if($row['evaso']==1){echo "consegnato";}else{echo "non consegnato";} ?></td></tr><?php
                    }
                ?></tbody></table><?php
            } else{
              ?><p>Nessun ordine effettuato.</p><?php
            }
             ?>
        </div>
      </div>
